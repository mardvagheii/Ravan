...
<header class="top-header shadow-sm" style="background:{{$settings->colorPrimary}}!important;">
        <div class="container">
            <nav class="top-nav d-flex flex-wrap align-items-center">
                ...
                <div class="logo mr-xl-0 mr-auto">
                    <img src="{{asset($settings->logo)}}" alt="logo">
                </div>
                <ul class="d-none d-xl-flex flex-wrap items">
                    <li><a href="{{ route('Web.index') }}">صفحه اصلی</a></li>
                    <li><a href="{{ route('Web.Blogs') }}">بلاگ</a></li>
                    <li><a href="#">دانلود اپلیکیشن</a></li>
                    <li><a href="{{ route('Web.Category') }}">دسته بندی های مشاوره</a></li>
            ...
            <div id="top-footer">
                <div id="footer-child" class="row">
                    <div class="col-md-4" id="right-footer-top">
                        <img id="img-footer-top" style="max-width: 100%; height: 150px;" src="{{asset($settings->logo)}}" alt="logo">
                        <div id="parent-link">
                        ...
                        <div class="d-flex justify-content-center">
                            @forelse ($Footer['trust']['items'] as $item)
                                <a href="{{ isset($item['trust_link']) ? $item['trust_link'] : '#'  }}">
                                    <img   style="height: 115px;object-fit: contain; max-width: 100%;" src="{{ asset(isset($item['trust_image']) && !is_array($item['trust_image'])  ? $item['trust_image'] : '' ) }}" alt="عکسی ثبت نشده است">
                                </a>
                            @empty
                                <h4> </h4>
                            @endforelse
                        </div>
                        <p id="footer-up-content-p">{{ isset($Footer['trust']['detail']) ? $Footer['trust']['detail'] : '' }}</p>
                    </div>
                    ...
                    <div class="col-md-4" id="left-footer-top">
                        ...
                        <div id="icons">
                            @forelse ($Footer['social_media'] as $item)
                                <a id="" href="{{ isset($item['social_media_link']) ? $item['social_media_link'] : '#'  }}" style="margin:10px 20px; background-size: contain; background-image: url({{ asset(isset($item['social_media_image']) && !is_array($item['social_media_image'])  ? $item['social_media_image'] : '' ) }});" alt="عکسی ثبت نشده است" ></a>
                            @empty
                                <h4> </h4>
                            @endforelse
                        </div>
